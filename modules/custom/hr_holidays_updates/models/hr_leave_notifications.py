...
class HrLeaveAllocationNotifications(models.Model):
    ...
    def _notify_employee(self, body: str):
        ...
        for rec in self:
            ...
            if not user:
                continue
            rec._hrmis_push(user, "Allocation request update", body)
    ...
    def _approver_users(self):
        ...
        # OpenHRMS multi-level approval (if installed for allocations)
        if "validation_status_ids" in self._fields:
            try:
                statuses = self.validation_status_ids.filtered(lambda s: not getattr(s, "validation_status", False))
                users |= statuses.mapped("user_id")
            except Exception:
        ...
        for rec in self:
            if rec.state == "confirm":
                rec._notify_approvers(
                    f"New allocation request from {rec.employee_id.name or 'an employee'} needs approval."
                )
        return res
