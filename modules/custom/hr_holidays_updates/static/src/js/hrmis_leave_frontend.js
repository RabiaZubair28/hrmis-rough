...
function _syncDateInputsEnabledAndRange(formEl) {
  ...
  if (!lprTypeId || leaveTypeId !== lprTypeId) {
    ...
    const minDefault = dateFromEl.getAttribute("data-default-min") || dateFromEl.min;
  ...
  if (!dobUtc) {
    ...
    _showInlineAlert(formEl, "error", "Date of birth is required to apply for LPR leave.");
    return;
  }
  ...
  const minUtc = startAllowed.getTime() > tomorrowUtc.getTime() ? startAllowed : tomorrowUtc;
